    ...
    do i = 1, dna.n_strand
        do j = 1, dna.strand(i).n_base
            if(dna.top(dna.strand(i).base(j)).xover /= -1) then
                ...
                write(701, "(3f9.3$)"), pos_1(1:3)
                write(701, "(3f9.3$)"), pos_2(1:3)
                if(dna.strand(i).type1 == "scaf") write(701, "(1f9.3 )"), radius*1.04d0
                if(dna.strand(i).type1 == "stap") write(701, "(1f9.3 )"), radius*1.05d0
            end if
        end do
    end do
...
subroutine Output_Write_Out_Sequences(prob, mesh, dna, unit)
    ...
    integer :: i, j, ii, base, n_scaf, n_stap, dozen, add
...
subroutine Output_Write_Out_JSON(prob, geom, mesh, dna, max_unpaired)
    ...
    shift = para_start_bp_ID + 21 * ((abs(min_bp)+max_unpaired)/21 + 1)

    ! Possible maximum edge length = max_bp - min_bp + 1 + max_unpaired
    width = shift + (max_bp - min_bp + 1 + max_unpaired)
    width = (width / 21 + 1) * 21
