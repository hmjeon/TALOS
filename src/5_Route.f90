...
!                                                                    Updated : 2017/04/29
!
! Comments: This module is to define the function for the scaffold routing.
!
...
subroutine Route_Chimera_Route(prob, geom, mesh, dna, step_route)
    ...
    do i = 1, dna.n_base_scaf
        ...
        if(node_1 == -1 .or. node_2 == -1) then
            ...
            write(601, "(3f9.3$)"), pos_1(1:3) * 10.0d0
            write(601, "(3f9.3$)"), pos_2(1:3) * 10.0d0
            write(601, "(1f9.3 )"), 0.1d0 * 10.0d0
        ...
        if(dna.base_scaf(base_1).xover /= -1) then
            ...
            write(601, "(a$    )"), ".cylinder "
            write(601, "(3f9.3$)"), pos_1(1:3) * 10.0d0
            write(601, "(3f9.3$)"), pos_x(1:3) * 10.0d0
            write(601, "(1f9.3 )"), 0.1d0 * 10.0d0
    ...
    do i = 1, dna.n_base_stap
        ...
        if(node_1 == -1 .or. node_2 == -1) then
            ...
            write(602, "(3f9.3$)"), pos_1(1:3) * 10.0d0
            write(602, "(3f9.3$)"), pos_2(1:3) * 10.0d0
            write(602, "(1f9.3 )"), 0.1d0 * 10.0d0
        ...
        if(dna.base_stap(base_1).xover /= -1) then
            ...
            write(602, "(a$    )"), ".cylinder "
            write(602, "(3f9.3$)"), pos_1(1:3) * 10.0d0
            write(602, "(3f9.3$)"), pos_x(1:3) * 10.0d0
            write(602, "(1f9.3 )"), 0.1d0 * 10.0d0
    ...
    if(f_dir == .true.) then
        do i = 1, geom.n_croL
            ...
            write(601, "(a     )"), ".color red"
            write(601, "(a$    )"), ".arrow "
            write(601, "(3f8.2$)"), pos_1(1:3) * 10.0d0
            write(601, "(3f8.2$)"), (pos_1(1:3) + vec(1:3)*1.3d0) * 10.0d0
            write(601, "(3f8.2 )"), 1.2d0, 3.0d0, 0.6d0

            write(601, "(a$    )"), ".arrow "
            write(601, "(3f8.2$)"), (pos_2(1:3) - vec(1:3)*1.3d0) * 10.0d0
            write(601, "(3f8.2$)"), pos_2 * 10.0d0
            write(601, "(3f8.2 )"), 1.2d0, 3.0d0, 0.6d0
            ...
            write(602, "(a     )"), ".color red"
            write(602, "(a$    )"), ".arrow "
            write(602, "(3f8.2$)"), pos_1(1:3) * 10.0d0
            write(602, "(3f8.2$)"), (pos_1(1:3) + vec(1:3)*1.3d0) * 10.0d0
            write(602, "(3f8.2 )"), 1.2d0, 3.0d0, 0.6d0

            write(602, "(a$    )"), ".arrow "
            write(602, "(3f8.2$)"), (pos_2(1:3) - vec(1:3)*1.3d0) * 10.0d0
            write(602, "(3f8.2$)"), pos_2 * 10.0d0
            write(602, "(3f8.2 )"), 1.2d0, 3.0d0, 0.6d0
        end do
    end if
...
subroutine Route_Reconnect_Junction(geom, bound, mesh, dna)
    ...
    integer :: n_col, start, n_scaf, n_stap, n_face, n_conn
    integer :: i, j, k, m, n, n_add_base_scaf, n_add_base_stap, n_un_scaf, n_un_stap
    logical :: b_conn
...
function Route_Connect_Scaf(mesh, dna, node_cur, node_com) result(n_add_base)
    ...
    double precision :: vec_in(3), vec_out(3), angle
    integer :: node_in, node_out, node_in_dn, node_out_up
    integer :: i, cur, com, ttt, n_add_base, one_nt
    ...
    one_nt = 0
    if(para_un_depend_angle == "on") then
        if(mesh.node(cur).sec == 2 .or. mesh.node(cur).sec == 3 .or. mesh.node(cur).sec == 4 .or. mesh.node(cur).sec == 5) then
            one_nt = 1
        end if
    end if

    ! ==========  DEPENDING ON ANGLE BETWEEN TWO EDGES  ==========
    ...
    if(para_unpaired_scaf == "on" .and. mesh.node(cur).conn == 3 .and. mesh.node(com).conn == 3) then
        ...
        ! ==========  DEPENDING ON ANGLE BETWEEN TWO EDGES  ==========
        if(angle * 180.0d0 / pi < 61.0d0) then
            !print *, angle * 180.0d0 / pi, "****"
            n_add_base = n_add_base + one_nt
        end if
        ! ==========  DEPENDING ON ANGLE BETWEEN TWO EDGES  ==========
    else
...
function Route_Connect_Stap(mesh, dna, node_cur, node_com) result(n_poly_Tn)
    ...
    double precision :: vec_in(3), vec_out(3), angle
    integer :: node_in, node_out, node_in_dn, node_out_up
    integer :: i, cur, com, ttt, n_poly_Tn, one_nt

    if(para_un_depend_angle == "on")  one_nt = 1
    if(para_un_depend_angle == "off") one_nt = 0
...
! Set node and edge for dual graph
subroutine Route_Graph_Set_Data1(prob, mesh, dna, pos_node, tail, head, cost)
    type(ProbType),   intent(in)    :: prob
    type(MeshType),   intent(in)    :: mesh
    type(DNAType),    intent(in)    :: dna
    double precision, intent(inout) :: pos_node(:,:)
    ...
    ! Deallocate memory
    deallocate(n_base)
end subroutine Route_Graph_Set_Data1
...
subroutine Route_Graph_Chimera_Spanning_Tree(prob, pos_node, tail, head, tree)
    ...
    write(606, "(a)"), ".color black"
    do i = 1, n_node
        write(606, "(a$     )"), ".sphere "
        write(606, "(3f10.3$)"), pos_node(i, 1:3) * 10.0d0
        write(606, "(1f9.3  )"), 0.35d0 * 10.0d0
    end do
    ...
    do i = 1, n_edge
        ...
        if(b_branch == .true.) then
            ...
            write(606, "(a)"), ".color dodger blue"
            radius = 0.1d0
        end if

        write(606, "(a$     )"), ".cylinder "
        write(606, "(3f10.3$)"), pos_1(1:3) * 10.0d0
        write(606, "(3f10.3$)"), pos_2(1:3) * 10.0d0
        write(606, "(1f9.3  )"), radius * 10.0d0
    end do
    ...
    ! Write nodes
    do i = 1, n_node
        write(606, "(3f10.3$)"), pos_node(i, 1:3)
...
subroutine Route_Find_Possible_Stap_Xover(geom, mesh, dna)
    ...
    ! Find maximum and minimum basepair ID in cross-sectional edges, croL
    do i = 1, mesh.n_node
        croL_cur = mesh.node(i).croL
        id_bp    = mesh.node(i).bp
