...
!                                                                    Updated : 2017/04/29
!
! Comments: This module is to define the function for the basepair model.
!
...
module Basepair
    ...
    private Basepair_Increase_Edge_Old
...
subroutine Basepair_Chimera_Cylinder(prob, geom, bound, mesh, mode)
    ...
    if(mode == "cylinder_1" .or. (mode == "cylinder_2" .and. f_mitered == .false.)) then
        ...
        do i = 1, geom.n_croL
            ...
            !if(geom.croL(i).iniL /= 1 .and. geom.croL(i).iniL /= 2) cycle

            pos_1(1:3) = geom.croP(geom.croL(i).poi(1)).pos(1:3)
            pos_2(1:3) = geom.croP(geom.croL(i).poi(2)).pos(1:3)

            write(502, "(a$     )"), ".cylinder "
            write(502, "(3f10.3$)"), pos_1(1:3) * 10.0d0
        ...
        do i = 1, geom.n_croL
            ...
            if(Is_Same_Vector(pos_1, pos_2) == .false.) then
                ...
                write(502, "(a$     )"), ".cylinder "
                write(502, "(3f12.5$)"), pos_1(1:3) * 10.0d0
                write(502, "(3f12.5$)"), pos_2(1:3) * 10.0d0
                write(502, "(1f9.3  )"), radius * 10.0d0
            end if
    ...
    if(mode == "cylinder_1") then
        ...
        do i = 1, bound.n_junc
            do j = 1, geom.n_sec * bound.junc(i).n_arm
                ...
                write(502, "(3f10.3$)"), pos_1(1:3) * 10.0d0
                write(502, "(3f10.3$)"), pos_2(1:3) * 10.0d0
                write(502, "(1f9.3  )"), radius*0.1d0 * 10.0d0

                ! Write sphere to represent start and end edge
                write(502, "(a      )"), ".color steel blue"
                write(502, "(a$     )"), ".sphere "
                write(502, "(3f10.3$)"), pos_1(1:3) * 10.0d0
                write(502, "(1f9.3  )"), 0.2d0 * 10.0d0
                write(502, "(a$     )"), ".sphere "
                write(502, "(3f10.3$)"), pos_2(1:3) * 10.0d0
                write(502, "(1f9.3  )"), 0.2d0 * 10.0d0
            end do
    ...
    if(mode == "cylinder_1") then
        ...
        do i = 1, geom.n_iniP
            write(502, "(a$    )"), ".sphere "
            write(502, "(3f9.2$)"), geom.iniP(i).pos(1:3) * 10.0d0
            write(502, "(1f9.2 )"), 0.5d0 * 0.6d0 * 10.0d0
        end do
        ...
        do i = 1, bound.n_junc
            do j = 1, bound.junc(i).n_arm
                ...
                write(502,"(a$    )"), ".cylinder "
                write(502,"(3f9.2$)"), pos_1(1:3) * 10.0d0
                write(502,"(3f9.2$)"), pos_2(1:3) * 10.0d0
                write(502,"(1f9.2 )"), 0.2d0 * 0.6d0 * 10.0d0
            end do
        end do
    end if
    ...
    do i = 1, bound.n_junc
        ...
        do j = 1, n_conn
            ...
            if(type_conn(j) == 2) then
                ...
                if(para_mitered_method == "new") then
                    call Basepair_Increase_Edge(geom, bound, mesh, node_cur, node_com)
                else
                    call Basepair_Increase_Edge_Old(geom, bound, mesh, node_cur, node_com)
                end if
            end if
        end do
...
! Increase edge length for neighbor connection
subroutine Basepair_Increase_Edge_Old(geom, bound, mesh, node_cur, node_com)
    type(geomType),  intent(inout) :: geom
    type(BoundType), intent(inout) :: bound
    type(MeshType),  intent(inout) :: mesh
    integer,         intent(in)    :: node_cur
    ...
    vec_in(1:3)  = mesh.node(node_in).pos  - mesh.node(node_in_dn).pos
    vec_out(1:3) = mesh.node(node_out).pos - mesh.node(node_out_up).pos
    angle        = dacos(dot_product(vec_in, vec_out)/(Norm(vec_in)*Norm(vec_out)))
...
end subroutine Basepair_Increase_Edge_Old
